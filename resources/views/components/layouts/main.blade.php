...
<head>
    ...
    <style>
        ...
        body { direction: {{ in_array(app()->getLocale(), ['ku', 'ar']) ? 'rtl' : 'ltr' }}; }
...
<body class="flex flex-col min-h-screen bg-dark-50 text-dark-900 antialiased">
            ...
            <div class="flex justify-between items-center h-14 sm:h-16 lg:h-20">
                ...
                <div class="flex items-center gap-2 sm:gap-4 flex-1">
                    ...
                    <div class="flex-1 max-w-md mx-2 lg:hidden">
                        <div class="relative">
                            <input type="text" 
                                   placeholder="{{ app()->getLocale() == 'ku' ? 'گەڕان...' : (app()->getLocale() == 'ar' ? 'بحث...' : 'Search...') }}"
                                   class="w-full h-8 sm:h-9 px-3 sm:px-4 {{ in_array(app()->getLocale(), ['ku', 'ar']) ? 'pr-10 pl-3' : 'pl-10 pr-3' }} bg-gray-100 border-0 rounded-full text-sm placeholder-gray-400 focus:ring-2 focus:ring-purple-300 focus:bg-white transition-all">
                            <div class="absolute {{ in_array(app()->getLocale(), ['ku', 'ar']) ? 'right-3' : 'left-3' }} top-1/2 -translate-y-1/2">
                                <i class="fa-solid fa-search text-gray-400 text-xs"></i>
                            </div>
                        </div>
                    </div>
                    
                    {{-- Language Switcher (Next to Logo) - Desktop Only --}}
                    ...
                    <div x-data="{ langOpen: false }" class="relative hidden lg:block">
                        ...
                        <div x-show="langOpen" 
                             ...
                             x-cloak
                             class="absolute {{ in_array(app()->getLocale(), ['ku', 'ar']) ? 'right-0' : 'left-0' }} mt-2 w-36 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
                            <a href="{{ route('language.switch', 'ku') }}" 
                               class="flex items-center justify-between px-4 py-3 hover:bg-purple-50 transition-all {{ app()->getLocale() == 'ku' ? 'bg-purple-50 text-purple-700 font-bold' : 'text-gray-700' }}">
                            ...
                            <a href="{{ route('language.switch', 'ar') }}" 
                               class="flex items-center justify-between px-4 py-3 hover:bg-purple-50 transition-all {{ app()->getLocale() == 'ar' ? 'bg-purple-50 text-purple-700 font-bold' : 'text-gray-700' }}">
                            ...
                            <a href="{{ route('language.switch', 'en') }}" 
                               class="flex items-center justify-between px-4 py-3 hover:bg-purple-50 transition-all {{ app()->getLocale() == 'en' ? 'bg-purple-50 text-purple-700 font-bold' : 'text-gray-700' }}">
                        ...
                        @admin
                            <li>
                                <a href="{{ route('dashboard') }}"
                                   ...
                                   {{ request()->routeIs('dashboard') ? 'text-primary-600 bg-primary-50' : 'text-dark-600 hover:text-primary-600 hover:bg-primary-50/50' }}">
                ...
                <div class="hidden lg:flex items-center gap-3">
                    @auth
                        <span class="text-sm text-dark-500">{{ __('messages.hello') }}، {{ auth()->user()->name }}</span>
                    ...
                    @else
                        <a href="{{ route('login') }}" class="px-5 py-2.5 rounded-xl font-medium text-dark-700 hover:text-primary-600 hover:bg-primary-50/50 transition-all text-sm">
                            {{ __('messages.login') }}
                        </a>
                        <a href="{{ route('register') }}" class="btn-primary text-white px-5 py-2.5 rounded-xl font-medium text-sm">
                ...
                <div class="flex items-center gap-3 lg:hidden">
                    ...
                    <div x-data="{ open: false }" class="relative sm:hidden">
                        ...
                        <div x-show="open" 
                             ...
                             x-transition:leave-end="opacity-0 scale-95"
                             x-cloak
                             class="absolute {{ in_array(app()->getLocale(), ['ku', 'ar']) ? 'left-0' : 'right-0' }} mt-2 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50 min-w-[120px]">
                            
                            <a href="{{ route('language.switch', 'ku') }}" 
                               class="flex items-center justify-between px-4 py-3 transition-all {{ app()->getLocale() == 'ku' ? 'bg-purple-50 text-purple-700 font-bold' : 'hover:bg-gray-50 text-gray-700' }}">
                                <span class="text-sm">کوردی</span>
                                @if(app()->getLocale() == 'ku')
                                    <i class="fa-solid fa-check text-purple-600 text-xs"></i>
                                @endif
                            ...
                            <a href="{{ route('language.switch', 'ar') }}" 
                               class="flex items-center justify-between px-4 py-3 transition-all {{ app()->getLocale() == 'ar' ? 'bg-purple-50 text-purple-700 font-bold' : 'hover:bg-gray-50 text-gray-700' }}">
                                <span class="text-sm">العربية</span>
                                @if(app()->getLocale() == 'ar')
                                    <i class="fa-solid fa-check text-purple-600 text-xs"></i>
                                @endif
                            ...
                            <a href="{{ route('language.switch', 'en') }}" 
                               class="flex items-center justify-between px-4 py-3 transition-all {{ app()->getLocale() == 'en' ? 'bg-purple-50 text-purple-700 font-bold' : 'hover:bg-gray-50 text-gray-700' }}">
                                <span class="text-sm">English</span>
                                @if(app()->getLocale() == 'en')
                                    <i class="fa-solid fa-check text-purple-600 text-xs"></i>
                                @endif
            ...
            <div class="max-w-7xl mx-auto px-4 py-6 space-y-2">
                    ...
                    <a href="{{ route($item['route']) }}" 
                       ...
                       {{ request()->routeIs($item['route']) 
                        ...
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center
                             {{ request()->routeIs($item['route']) ? 'bg-white/20' : 'bg-primary-50' }}">
                            <i class="fa-solid {{ $item['icon'] }} {{ request()->routeIs($item['route']) ? 'text-white' : 'text-primary-500' }}"></i>
                        </div>
                ...
                @auth
                    @admin
                        <a href="{{ route('dashboard') }}" @click="open = false"
                           class="flex items-center gap-4 p-4 rounded-2xl transition-all
                           {{ request()->routeIs('dashboard') ? 'bg-gradient-to-r from-primary-500 to-accent-500 text-white' : 'text-dark-600 hover:bg-dark-50' }}">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center {{ request()->routeIs('dashboard') ? 'bg-white/20' : 'bg-primary-50' }}">
                                <i class="fa-solid fa-gauge-high {{ request()->routeIs('dashboard') ? 'text-white' : 'text-primary-500' }}"></i>
                            </div>
                ...
                <div class="pt-4 mt-4 border-t border-dark-100">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <span class="font-medium text-dark-700">{{ __('messages.language') }}</span>
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                        <a href="{{ route('language.switch', 'ku') }}" 
                           class="flex items-center justify-center p-3 rounded-xl border-2 transition-all
                           {{ app()->getLocale() == 'ku' ? 'border-purple-500 bg-purple-50 text-purple-700 font-bold' : 'border-gray-200 hover:border-purple-300 text-gray-700' }}">
                            <span class="text-sm">کوردی</span>
                        </a>
                        <a href="{{ route('language.switch', 'ar') }}" 
                           class="flex items-center justify-center p-3 rounded-xl border-2 transition-all
                           {{ app()->getLocale() == 'ar' ? 'border-purple-500 bg-purple-50 text-purple-700 font-bold' : 'border-gray-200 hover:border-purple-300 text-gray-700' }}">
                            <span class="text-sm">العربية</span>
                        </a>
                        <a href="{{ route('language.switch', 'en') }}" 
                           class="flex items-center justify-center p-3 rounded-xl border-2 transition-all
                           {{ app()->getLocale() == 'en' ? 'border-purple-500 bg-purple-50 text-purple-700 font-bold' : 'border-gray-200 hover:border-purple-300 text-gray-700' }}">
                            <span class="text-sm">English</span>
                        </a>
                    </div>
                </div>
                ...
                <div class="pt-4 mt-4 border-t border-dark-100">
                    @auth
                        <div class="flex items-center justify-between p-4 bg-dark-50 rounded-2xl">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-medium text-dark-700">{{ auth()->user()->name }}</span>
                            </div>
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                    ...
                    @else
                        <div class="grid grid-cols-2 gap-3">
                            <a href="{{ route('login') }}" class="p-4 text-center rounded-2xl border-2 border-dark-200 font-medium text-dark-700 hover:border-primary-500 hover:text-primary-600 transition-all">
                                {{ __('messages.login') }}
                            </a>
                            <a href="{{ route('register') }}" class="p-4 text-center rounded-2xl btn-primary text-white font-medium">
                                {{ __('messages.register') }}
                            </a>
                        </div>
                    @endauth
    ...
    {{-- Main Content --}}
    <main class="@if ($pd == 'true') max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full @endif grow">
        {{ $slot }}
    </main>
    ...
    <footer class="bg-dark-900 text-white mt-auto relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 relative">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12">
                ...
                {{-- Quick Links --}}
                <div>
                    <h4 class="font-semibold mb-4 text-white">{{ __('messages.shop') }}</h4>
                    <ul class="space-y-3">
                        @php
                        ...
                        @foreach($shopLinks as $link)
                            <li><a href="{{ route('products.index') }}" class="text-sm text-dark-400 hover:text-primary-400 transition-colors">{{ $link }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                {{-- Support --}}
                <div>
                    <h4 class="font-semibold mb-4 text-white">{{ app()->getLocale() == 'ku' ? 'یارمەتی' : 'Support' }}</h4>
                    <ul class="space-y-3">
                        @php
                        ...
                        @foreach($supportLinks as $link)
                            <li><a href="#" class="text-sm text-dark-400 hover:text-primary-400 transition-colors">{{ $link }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                {{-- Contact --}}
                <div>
                    <h4 class="font-semibold mb-4 text-white">{{ __('messages.contact_us') }}</h4>
            ...
            <div class="mt-12 pt-8 border-t border-dark-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                <p class="text-sm text-dark-500">© {{ date('Y') }} {{ app()->getLocale() == 'ku' ? 'کورد مارکەت. هەموو مافەکان پارێزراون.' : 'Kurd Market. All rights reserved.' }}</p>
                <div class="flex items-center gap-6 text-sm text-dark-500">
                    <a href="#" class="hover:text-primary-400 transition-colors">{{ __('messages.privacy_policy') }}</a>
                    <a href="#" class="hover:text-primary-400 transition-colors">{{ __('messages.terms') }}</a>
                </div>
            </div>
        </div>
    </footer>
